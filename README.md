...
- To set up V1 for another candidate, edit only `app_properties.py` and replace `v1/Prompts/story.md`. Candidate identity, contact details, education, experience identities and aliases, project names and URLs, mode counts/orders, writing limits, coursework policy, and V1 stage budgets live in `app_properties.py`. Keep each configured project ID and name identical to its `story.md` heading; startup validation reports a clear mismatch.
...
```powershell
python manager.py Resume-word/[Configured_Resume_Stem]_Company_Resume.docx "Company Name"
```

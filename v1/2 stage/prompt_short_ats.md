...
- Validate the exact runtime compact V1 schema, including the absence of `employment_note` and other extra fields. Independently count every final bullet with whitespace-separated words and flag every bullet above {{V1_HARD_MAXIMUM_WORDS}} words.
- Audit `coursework` after `summary`: when enabled by the selected runtime mode, it uses only runtime-verified titles and obeys runtime `coursework_selection`; otherwise it is `[]`.
- Audit the selected runtime mode's exact project-bullet count.
- Do not require a compact GPA field. The runtime supplies the configured verified GPA only when the selected mode enables it.
...
- Audit outcomes semantically using the runtime performance-outcome and essential-scope limits. Treat a before-and-after comparison as one outcome. Flag stacked speed, quality, reliability, accuracy, efficiency, or delivery outcomes, and specify which outcome best supports the JD within those limits.
...
- Recommend only safe same-slot repairs. Any suggested bullet must itself be natural, evidence-locked, and no more than the runtime hard maximum. Never give the Optimizer an instruction that violates the V1 contract.

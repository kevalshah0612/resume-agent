...
- Write one bullet per selected project only for `mid_swe`, where TCS is split into `TCS_SWE_II` and `TCS_SWE_I`. Keep two bullets per project for entry modes, including `entry_aiml` with `TCS_COMBINED`.

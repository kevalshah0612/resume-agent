...
You are creating one targeted resume JSON for Keval Shah for one specific job description.
...
Goal:
Produce a recruiter-screenable, ATS-parseable, interview-defensible resume JSON that proves Keval has done the closest version of the target job using only authenticated evidence.
...
`A recruiter should call Keval because [JD-core system] + [exact JD stack] + [production/project proof] + [risk reducer]`
...
1. Has Keval done the closest version of this job?
...
Project routing:
- JobPulse: backend, full-stack, job data, Kafka, PostgreSQL, React dashboards
- FraudSift: fraud, transaction analysis, FastAPI, Docker, risk dashboards
- ReviewBot: code review, developer tools, GitHub Actions, AI-assisted review
- FilingQuery: RAG, document search, SEC filings, embeddings, retrieval
- EvalTrace: LLM evaluation, hallucination reduction, RAG reliability
- Resume Agent: AI tooling, resume automation, DES approval, recruiter review, JSON validation, DOCX/PDF pipeline
- JobFill AI Extension: Chrome extension, Workday automation, form autofill, local storage, Claude custom questions, browser automation
...
Do not overexplain the Sep 2020 to Mar 2021 gap. It is not a serious resume issue.
...
`JobPulse | FraudSift | ReviewBot | FilingQuery | EvalTrace | Resume Agent | JobFill AI Extension`
...
```json
{
  ...
  "projects": [
    {
      "name": "JobPulse | FraudSift | ReviewBot | FilingQuery | EvalTrace | Resume Agent | JobFill AI Extension",
